...
** Compile src/*
=src/= is for single file non-script source code, scripts go to =sh/=. Both are in =$PATH=. They are compiled with a makefile, compiling takes a short time since it is only simple C programs so far. The programs have no dependencies, outside of base that is.
#+BEGIN_SRC sh
